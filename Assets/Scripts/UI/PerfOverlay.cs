...
public class PerfOverlay : MonoBehaviour {
  ...
  void Start(){
    var canvas = Object.FindObjectOfType<Canvas>(); if(canvas==null)
    {
      return;
    }

    var go=new GameObject("PerfText"); go.transform.SetParent(canvas.transform,false);
    _text=go.AddComponent<Text>(); _text.font=Resources.GetBuiltinResource<Font>("LegacyRuntime.ttf"); if(_text.font==null)
    {
  ...
  void Update(){
    _accum+=Time.unscaledDeltaTime; _frames++; if(_accum>=0.5f && _text!=null){ var fps=_frames/_accum;
      var p=EcosystemManager.Instance?.GetCount("Producers")??0; var h=EcosystemManager.Instance?.GetCount("Herbivores")??0; var c=EcosystemManager.Instance?.GetCount("Carnivores")??0;
